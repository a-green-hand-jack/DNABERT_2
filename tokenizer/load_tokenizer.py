...
if __name__ == "__main__":
    ...
    print(dna_tokenizer.pad_token_id, dna_tokenizer.pad_token)
    print(dna_tokenizer.cls_token_id, dna_tokenizer.cls_token)
    print(dna_tokenizer.unk_token_id, dna_tokenizer.unk_token)
    print(dna_tokenizer.mask_token_id, dna_tokenizer.mask_token)
    print(dna_tokenizer.sep_token_id, dna_tokenizer.sep_token)

    # 加载DNA 数据集
    train_file_path, eval_file_path = "../../Datasets/Human_genome/huixin/train.txt", "../../Datasets/Human_genome/huixin/eval.txt"
    ...
    dna_sequence_list = [dna_train_dataset.__getitem__(0), dna_train_dataset.__getitem__(1), dna_train_dataset.__getitem__(2)]
    processed_data = data_collator(dna_sequence_list)
    # 输出处理后的数据
    # print(processed_data)
    # # 解码处理后的数据
    decoded_text = dna_tokenizer.batch_decode(processed_data['input_ids'])
    # 打印结果
    # print(decoded_text)
    ...
    # 加载模型
    # model = BertForMaskedLM.from_pretrained(pretrained_model_name_or_path="../zhihan1996/DNA_bert_6")
    # 实例化一个空的BERT模型
    ...
    print(output)
    # 计算损失
    # loss = loss_function(output.prediction_logits, labels)
    # print(output.loss)
    # 打印嵌入层维度
    embedding_dimension = model.config.hidden_size
    print("嵌入层维度:", embedding_dimension)

    # 打印词嵌入维度
    vocab_size = len(dna_tokenizer)
    print("词嵌入维度:", embedding_dimension)

    # 打印词汇表大小
    print("词汇表大小:", vocab_size)

    # 打印填充标记的索引
    padding_token_index = dna_tokenizer.pad_token_id
    print("填充标记的索引:", padding_token_index)
